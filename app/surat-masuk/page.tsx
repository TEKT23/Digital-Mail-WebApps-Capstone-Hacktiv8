...
import suratMasukData from "@/data/suratMasuk";
...
export default function SuratMasukPage() {
  ...
  const [suratMasuk, setSuratMasuk] = useState<SuratMasuk[]>(suratMasukData);
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${config.class}`}
      >
        {config.icon}
  ...
  const submitDisposisi = () => {
    ...
    // Update the status of the selected surat

    if (selectedSurat) {
      const updatedSuratList = suratMasuk.map((surat) =>
        surat.id === selectedSurat.id
          ? { ...surat, status: 'disposisi' as SuratMasuk['status'], disposisiKe: disposisiData.tujuan }
          : surat
      );
  ...
  const filteredSurat = suratMasuk.filter((surat) => {
    ...
    const matchFilter =
      filterStatus === "semua" || surat.status === filterStatus;
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2 ml-4">
                  <button
                    onClick={() => router.push(`/surat-detail?id=${surat.id}`)}
                  ...
                  {(userRole === "bagian-umum" || userRole === "direktur") &&
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 border-t border-neutral-200 flex justify-end space-x-3">
              ...
              <button onClick={submitDisposisi} className="btn-primary">
                Kirim Disposisi
              </button>
            </div>
          </div>
